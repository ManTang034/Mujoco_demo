...
def initialize_environment(model_path, render_mode):
    ...
    try:
        env = PusherEnv(render_mode=render_mode, xml_file=model_path)
        env = Monitor(env)
        check_env(env)
        return env
...
def train_model(model, env, total_timesteps, model_path, save_path):
    ...
    try:
        ...
        #     env,
        #     best_model_save_path=os.path.join(model_path, "ppo_best/"),
        #     eval_freq=1000,
        #     deterministic=True,
        #     render=False,
        #     warn=False,
        ...
        model.save(os.path.join(save_path, "sac_push_v5"))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train a PPO model for PusherEnv.")
    parser.add_argument(
        "--model_path",
        type=str,
