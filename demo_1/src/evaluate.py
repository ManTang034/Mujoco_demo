...
def save_gif_async(frames, output_dir):
    ...
    gif_path = os.path.join(output_dir, "test.gif")
    ...
    for step in range(num_steps):
        ...
        if render_mode == "rgb_array":
            ...
            if step % frame_interval == 0:
                frames.append(image)
            cv2.imshow("Ball Balance Environment", image)
            cv2.waitKey(1)
...
def main(args):
    ...
    try:
        ...
        env = BallBalanceEnv(render_mode=args.render_mode, model_path=args.assets_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default=os.path.join(project_root, "data/models/ppo_ball_balance.zip"),
